var input = require('fs').readFileSync('./STRINGS/1020/stdin', 'utf8')
...
for (let i = 0; i < lines.length; i++) {
  ...
  teclaQuebrada = valores[0]
  frase = valores[1]

  if (teclaQuebrada === '0' && frase === '0') {
    novaFrase = '0'
  } else {
    do {
      if (frase[j] != teclaQuebrada) {
        novaFrase += frase[j]
      }
      j++
  ...
  if (novaFrase === '') {
    ...
    console.log(parseInt(novaFrase))
  }
  novaFrase = ''
}
