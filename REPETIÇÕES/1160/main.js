var input = require('fs').readFileSync('./REPETIÇÕES/1160/stdin', 'utf8')
...
for (let i = 1; i <= vezes; i++) {
  ...
  populacaoA = parseInt(valores[0])
  populacaoB = parseInt(valores[1])
  crescimentoA = parseFloat(valores[2])
  crescimentoB = parseFloat(valores[3])

  do {
    populacaoA += parseInt((populacaoA / 100) * crescimentoA) //Recebendo a porcentagem de crescimento
    populacaoB += parseInt((populacaoB / 100) * crescimentoB)
    cont++ //Sempre que reeber irá contar mais um ano
    if (cont > 100) {
      break //Se for maior que 100 irá pular para o final
    }
  } while (populacaoA <= populacaoB) //para quando A for maior que B
